...
const occupationsMetadata = {
    ...
    branches: [
      {
        name: '盾战士',
        value: '盾',
        description: '身穿板甲，手持塔盾，如钢铁壁垒般为队友承受伤害。虽行动迟缓，却能为后方提供无与伦比的安全保障。',
        attributes: `
    ...
    branches: [
      ...
      },
      {
        ...
        attributes: [`
          ...
          <b>雷霆回复：</b>自身受到雷电伤害后将1/2的伤害转化为回复
        `,`
      ...
      },
      {
        ...
        attributes: [`
          <b>甘露赐福：</b>次要动作，将潮汐之神的赐福锁定在6m内的一名友方目标身上，每天可以切换感知调整值次数。5级成长为3名目标、9级成长为6名目标<br/>
          <b>引导潮汐：</b>次要动作，对6m内目标引导潮汐能量，使其受到的潮汐领域神术持续回合+1，叠加回合数最多不超过自身等级<br/>
      ...
      },
      {
        ...
        attributes: [`
          <b>保护能量：</b>每天拥有等级*感知调整值的可施放护盾量，该护盾可以抵挡物理伤害或魔法伤害。主要动作对3m内的目标施放，可以自由分配每次施放的护盾量，也可以使用主要动作收回保护能量。每天可以使用感知调整值的次数。（保护能量可以对负血目标施放，释放后可以吸收生命流逝的伤害）<br/>
  ...
  paladin: {
    description: ['圣骑士是秩序和善良的守护者、是混乱和邪恶的制裁者、是伤痛和诅咒的净化者。', '他们有着高贵的灵魂和伟大的理想，他们遵守、践行和守护自己的神圣誓言，用利剑和生命与邪恶进行着不懈的斗争。在残酷和混乱的世界中，圣骑士就像一座灯塔，为迷途的盟友们照亮前程。', '圣骑士需要遵守、践行和守护自己的神圣誓言，誓言会赋予圣骑士强大的力量，但也会约束立誓者的一言一行。一旦违背誓言和承诺，圣骑士将丧失一切职业能力和职业天赋'],
    branches: [
      {
        name: '守护圣骑士',
        value: '守护',
        ...
        attributes: `
          ...
          <b>挑衅：</b>使用次要动作，对6m内一个敌人进行嘲讽勇气光环：圣骑士获得自身等级的神圣护盾
        `
      },
      {
        name: '裁决圣骑士',
  ...
  monk: {
    description: ['作为大陆上最晚被发现而又自成体系一个职业，武僧往往高深莫测。他们代表了肉体的磨练与精神的苦修。曾经被人划为了极度危险的异教徒受到了一众神明信徒的打压。致使他们不得不隐退至人烟稀少的地方。', '但是对于他们来说，外界的世俗只是精神与肉体磨练也是修成正果的阶梯。这些外放于拳腿，举戈而舞。内敛于丹田，凝神自安的武僧们也为遥远的东方大陆披上了一层神秘的面纱。'],
    branches: [
      {
        name: '拳腿宗',
        value: '拳腿宗',
        ...
        attributes: `
          <b>疾风连打（拳腿宗）：</b>武僧可以使用主要动作进行两次攻击（两次命中闪避对抗）<br/>
          <b>疾行：</b>武僧移动速度增加3m
        `
      },
      {
        ...
        attributes: `
          <b>外器棍法-横扫（外器宗）：</b>武僧可以使用一个主要动作，针对敌人下盘攻击，命中-2。命中后，造成伤害的同时与目标力量对抗敏捷/体质，豁免失败，则被击倒<br/>
          <b>疾行：</b>武僧移动速度增加3m
        `
      },
      {
...
const druidSpells = [
  [
    {
      name: "制造微风（风）",
      effect: "可以制造一阵微风，吹走极小的生物和气体。元素增强：制造小风环境",
      action: {
        isPrimary: true,
        meta: "单手，咒语"
      },
    ...
    {
      name: "创造纯水（水）",
      effect: "创造200ml（0.02寸方米）的纯净水。元素增强：可以创造200ml/等级的纯净水",
      action: {
        isPrimary: true,
        meta: "单手、咒语"
      },
...
dialogSelectInputs.forEach((dialogSelectInput) => {
  dialogSelectInput.addEventListener('touchstart', function(e) {
    ...
    setTimeout(() => { viewport.content = 'width=device-width, initial-scale=1'; }, 100);
    e.target.click();
  });
...
function createFaithOptions(availableDeities, onSelect) {
  ...
  container.innerHTML = availableDeities.map(deity => `
    <div class="card">
      ...
      <div>
        <h4>${deityMetadata[deity].title}——${deityMetadata[deity].name}</h4>
        <div>
