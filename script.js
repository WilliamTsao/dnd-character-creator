...
dialogContents.forEach((dialogContent) => {
  ...
  dialogContent.querySelectorAll('button').forEach((button) => {
    button.addEventListener('click', (e) => {
      ...
      if (category === 'race') {
        const race = raceCnToEn[e.target.dataset.value];

        // enable bloodline input and reset selected if race selection was changed
        const booldlineOptionSet = document.querySelector(`#bloodline-dialog > .${race}`);
        const bloodlineInput = document.querySelector('input#bloodline');
        bloodlineInput.value = '';
        dialog.querySelectorAll('#bloodline-dialog > div').forEach((type) => {
          type.style.display = 'none';
        });
        if (booldlineOptionSet) {
          booldlineOptionSet.style.display = 'block';
        ...
        // enable occupation input and reset selected if current is not longer available
        const availableOccupations = statsByRace[race].occupations;
        ...
        let occupationInputValue = occupationInput.value;
        if (occupationInputValue && occupationInputValue.includes('(')) {
          occupationInputValue = occupationInputValue.split('(')[0];
        }
        const currentSelectedOccupation = occupationCnToEn[occupationInputValue];
        if (!currentSelectedOccupation || !availableOccupations.includes(currentSelectedOccupation)) {
        ...
        if (age < statsByRace[race].age.min || age > statsByRace[race].age.max) {
          ageInput.value = '';
        }
        ageInput.min = statsByRace[race].age.min;
        ageInput.max = statsByRace[race].age.max;
        ageInput.placeholder = `输入${ageInput.min}~${ageInput.max}`;

        const heightInput = document.querySelector('input#height');
        const height = parseInt(heightInput.value);
        if (height < statsByRace[race].height.min || height > statsByRace[race].height.max) {
          heightInput.value = '';
        }
        heightInput.min = statsByRace[race].height.min;
        heightInput.max = statsByRace[race].height.max;
        heightInput.placeholder = `输入${heightInput.min}~${heightInput.max}`;

        const weightInput = document.querySelector('input#weight');
        const weight = parseInt(weightInput.value);
        if (weight < statsByRace[race].weight.min || weight > statsByRace[race].weight.max) {
          weightInput.value = '';
        }
        weightInput.min = statsByRace[race].weight.min;
        weightInput.max = statsByRace[race].weight.max;
        weightInput.placeholder = `输入${weightInput.min}~${weightInput.max}`;

        const speedInput = document.querySelector('input#speed');
        speedInput.value = statsByRace[race].speed;

        // set race specific skills
        const skills = document.querySelector(`#race-dialog .card.${race} .accordion.skill .accordion-content`).innerHTML;
        document.querySelector('.page4 .race-skills').innerHTML = skills;
      }
